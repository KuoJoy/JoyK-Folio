 (function ($){
    ...
    $.fn.aniEvent = function () {
        ...
        // leadermonthLink
        $('.leadermonthLink').load('/event_site/leadermonth/leadermonth_link.html');
        ...
        function onScroll(event) {
            ...
            //Sec5加上Fixed
            if (scrollPos >= $('#section-feedback').position().top && (scrollPos - 250 + $(window).height()) <= $('#Sec51').position().top) {
    ...
    $('.meddic__top').click(
        function(){//:hidden 匹配所有的不可見元素，input元素的type屬性為"hidden"的話也會被匹配到
            if ($(this).next().is(":hidden")){//next是下一層;is() 根據選擇器、元素或jQuery 對象來檢測匹配元素集合(代表一種狀態)；例如 is(":checked") 或 is(":animated")
                $('.class__toggle').removeClass('class__toggle--ani');
                $(this).find('.class__toggle').addClass('class__toggle--ani');
                $(this).next().slideDown(function(){               
                });
                $('.meddic__down').not($(this).next('.meddic__down')).slideUp(300);
                // $('.list_arrow').not($(this).find('.list_arrow')).transition({ rotate: '0deg' });  
            }else{
                $(this).find('.class__toggle').removeClass('class__toggle--ani');
